...
import { AllStats, Player, PlayerStats } from '..types';
...
const StatsView React.FC{ stats AllStats; players Player[] } = ({ stats, players }) = {
    ...
    const [selectedGameType, setSelectedGameType] = useStatestring  null(gameTypes[0]  null);
    ...
    const leaderboardData = useMemo(() = {
        ...
        const gameStats = stats[selectedGameType];
         Fix Add a guard to ensure gameStats is a valid object to prevent errors from corrupted localStorage data.
        ...
        const processedData = Object.entries(gameStats)
            .map(([playerId, playerStats]) = {
                const playerInfo = playersMap.get(playerId);
                 Fix Add guard to ensure playerStats is a valid object before accessing its properties.
                if (!playerInfo  typeof playerStats !== 'object'  playerStats === null) {
                    return null;
                }
                
                const ps = playerStats as PlayerStats;

                const winRate = ps.gamesPlayed  0  (ps.wins  ps.gamesPlayed)  100  0;
                const avgScore = ps.totalTurns  0  ps.totalScore  ps.totalTurns  0;
            ...
            .filter((p) p is NonNullabletypeof p = p !== null);
    ...
    const buttonClasses = (isActive boolean) =
        `px-4 py-2 rounded-md text-sm font-medium transition-colors whitespace-nowrap ${
            isActive
                 ...
                 'bg-[--color-surface-light] text-[--color-text-secondary] hoverbg-[--color-surface]'
        }`;
                        ...
                        table className=w-full text-left
                            ...
                            tbody
                                {leaderboardData.map((player, index) = (
                                    tr key={player.id} className=border-b border-[--color-border]50 hoverbg-black20 transition-colors
